...
### World Project (read-only resolver + workspace + preview shipped — Phase 4A/4B)
...
Still **not** implemented (each its own future approved lane): deterministic
**packaging** and any **upload**. The lane is strictly read-only: no path repair,
copy, rename, delete, or file mutation (the world preview only reads). See
`docs/WORLD_PROJECT_ARCHITECTURE.md`.
