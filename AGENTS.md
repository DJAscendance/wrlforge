...
- **Audit + package** the project into a portable review bundle (Phase 5A):
  `src/world-project/package-plan.js` derives a **deterministic** plan (packaged
  file set = primary + nested local WRL + present approved assets, each with
  ...
  never auto-included). **Build Review Bundle** (the one explicit write action)
  ...
  a not-for-upload label. Blocking rules: missing/case/absolute/traversal/remote/
  unreadable block; cycles don't. It refuses blocked/in-project/overwrite and
  re-hashes every file against the manifest. It is a **review bundle only** —
  **no direct upload**, no server-format claim (open questions in
  `docs/WORLD_PACKAGE_QUESTIONS.md`).

Still **not** implemented (each its own future approved lane): **direct upload**,
authentication, automatic path repair, copy, rename, delete, Apply/Bake, internal
editing, Windows packaging. Everything except the single Build-Review-Bundle action
is read-only; that action writes only a portable bundle to a caller-chosen
destination and never mutates the source project. See
